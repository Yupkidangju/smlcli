...
## ADR-025: Phase 25 Ultimate Polish & Security Hardening (v1.7.0)
...
### Context
v1.7.0 출시를 앞두고 시스템의 극한 한계 및 보안 샌드박싱에 대한 종합 점검(Phase 25)이 이루어졌습니다.
특히 터미널 크기 변동 시 패닉, 디스크 한계 용량 시 프로그램 크래시, 그리고 악의적인 경로(Path Traversal/Symlink)를 사용한 파일 접근이 가능할 수 있는 위험 요소들이 보고되었습니다.
또한, 백그라운드 프로세스가 실행된 이후 TUI로 복귀할 때 터미널 환경에 잔상(Ghosting)이 남아 UX를 해치는 현상도 있었습니다.

### Decision
1. **UTF-8 멀티바이트 안전성 (UX/UI)**: `unicode-width`를 도입하여 TUI 렌더링 시 바이트 단위 자르기로 인한 패닉을 방지하고 문자 단위/시각적 너비 단위 슬라이싱을 구현.
2. **심볼릭 링크 및 Path Traversal 방어 (Security)**: `std::fs::canonicalize`를 통해 사용자가 요청한 경로의 최종 절대 경로를 확인하고 `workspace_root`를 벗어나면 무조건 실행 차단.
3. **네트워크 지수 백오프 (Robustness)**: LLM API 호출(429/5xx) 및 응답 무한 지연을 방어하기 위해 `tokio::time::timeout` 60초 래퍼와 지수 백오프 기반 Retry 루프 도입.
4. **ENOSPC 디스크 폴백 (Reliability)**: 세션 로그 및 설정 파일 저장 시 `std::io::ErrorKind::StorageFull` 오류를 캡처하여 패닉을 억제하고 데이터 유실을 방지.
5. **터미널 잔상 제거 (UX)**: `ExecShell` 도구 사용 직후 `TerminalGuard::clear_and_reset()`을 호출하도록 `force_clear` 이벤트 루프를 동기화.

### Alternatives Considered
- **터미널 잔상 제거를 위해 별도의 Alt Screen 활용**: 그러나 서브 프로세스의 출력이 사용자의 터미널에 남기를 바라는 유스케이스가 있으므로 TUI 내부에서만 복구 처리.
- **`StorageFull` 발생 시 오래된 세션 삭제**: 사용자 동의 없이 데이터를 임의 삭제하는 것은 보안/정책상 부적절하므로 경고 노출 및 저장 보류 방식으로 결정.

### Consequences
- 극한 상황(네트워크 장애, 디스크 풀, 비정상적 터미널 비율)에서도 시스템 크래시 없이 안정적으로 운영됨.
- 샌드박스 공격 벡터 원천 차단 확인 (회귀 테스트를 통해 검증 완료).

---

## ADR-026: Phase 26 The Final Polish & Stability (v1.8.0)
...
### Context
v1.8.0을 위한 최종 폴리싱 단계에서 엣지 케이스 무결성 확보와 렌더링 성능 최적화가 요구되었습니다. 이진 데이터를 잘못 읽었을 때의 TUI 크래시, 청크(Chunk) 스트리밍 중 API Key 마스킹의 사각지대, 그리고 사용자가 Git 머지 충돌을 겪고 있는 도중에 AI가 체크포인트를 수행하여 Git 상태가 꼬이는 현상, 대화면 터미널에서의 CPU 오버헤드 등이 식별되었습니다.

### Decision
1. **바이너리 파일 감지**: 파일 입출력 도구 실행 시 첫 1KB를 사전 검사하여 `\0` 포함 여부 및 출력 불가능 문자 비율로 바이너리 파일을 필터링하고 중단합니다.
2. **슬라이딩 윈도우 마스킹 (Stateful Masking)**: 단순히 현재 청크만 정규식으로 검사하지 않고, 이전 청크의 끝부분 N 바이트를 이어붙인 가상 윈도우에서 패턴을 검사하여 잘린 API 키의 누출을 방지합니다.
3. **Git 충돌 상호 배제**: `.git/MERGE_HEAD`, `.git/REBASE_HEAD`, `.git/CHERRY_PICK_HEAD`의 존재를 확인하여 충돌 해결 상태일 때는 자동 체크포인트 기능을 잠시 보류합니다.
4. **TUI 렌더링 Row Caching**: `HashMap<usize, Line<'static>>` 형태의 캐시를 두어 터미널 리사이즈나 내용 변경이 없는 Tick에서는 구문 분석 및 스타일링 과정을 생략합니다.
5. **Provider 포맷 변환 추상화**: 내부 대화 상태는 Generic하게 유지하고, 전송 직전 Provider Adapter가 각 API(Anthropic, OpenAI 등) 규격에 맞게 찌꺼기 메시지(빈 Role 등)를 정제합니다.

### Alternatives Considered
- **TUI 전체 렌더링 캐싱**: 캐싱 메모리 관리 부담으로 인해 전체 화면 캐싱이 아닌 변경이 잦은 로그 라인별(Row-level) LRU 또는 Viewport 범위 캐싱만 제한적으로 적용.
- **스트리밍 마스킹용 정규식 고도화**: 단어 경계(`\b`) 정규식을 고도화하는 방법도 고려했으나, 바이트 스트림 레벨에서 경계가 잘려 들어오는 물리적 한계를 극복하기 위해 슬라이딩 윈도우 로직을 최종 채택함.

### Consequences
- Git이나 바이너리 관련 오작동으로 인한 개발자의 워크플로우 훼손(데이터 유실 등) 위험이 차단됨.
- 로그가 많이 쌓인 대화면 TUI에서도 CPU 점유율이 획기적으로 낮아져 쾌적한 반응성 제공.
- API Key 등 민감 정보가 100% 마스킹됨을 스트리밍 엣지 케이스까지 보장.

---

## ADR-027: Phase 27 Final Pre-Launch Hardening (v1.9.0)
...
### Context
v1.9.0 최종 릴리즈 준비 과정에서, 사용자가 실행한 쉘(Shell) 명령어가 생성한 백그라운드 프로세스가 메인 프로세스 취소 시에도 좀비/고아(Orphan) 상태로 남아 리소스를 점유하는 현상이 보고되었습니다. 또한 도구(Agent)가 실행 환경의 변수(API Key 등)에 의도치 않게 접근하거나 오염될 위험, 수백 MB 이상의 파일 덤프 시 메모리 폭발(OOM), 긴 실행 동안 터미널 제목이 갱신되지 않아 UX가 저하되는 문제가 식별되었습니다.

### Decision
1. **프로세스 그룹(PGID) 기반 완전 소멸**: 유닉스(`cfg(unix)`) 환경에서 도구 실행 시 `.process_group(0)`을 부여하고 취소 시 `libc::kill(-pgid, libc::SIGKILL)`을 호출하여 트리 내 모든 하위 프로세스를 완벽히 청소합니다.
2. **환경변수 격리(Whitelist)**: 도구 런타임에 `.env_clear()`를 적용 후 `PATH`, `HOME`, `USER`, `TERM`, `LANG` 등 최소한의 필수 변수와 Smlcli Config에 명시된 환경변수만 선택적으로 주입합니다.
3. **결과값 크기 캡핑 (Size Capping)**: 프로세스의 출력(`stdout`/`stderr`) 스트림이 누적 크기 한계(기본 5MB)를 초과하면 수신을 중단하고 잘린 부분에 생략 메타데이터(`... [중략] ...`)를 덧붙여 OOM을 차단합니다.
4. **터미널 OSC 상태 연동**: 도구 실행 시 터미널 탭 제목을 `\x1b]0;[smlcli] Executing: <tool_name>\x07`로 변경하고, 작업표시줄 진행 상태(OSC 9;4) 시퀀스를 통해 시스템 UI와 동기화합니다.

### Alternatives Considered
- **환경변수 Blacklist 방식**: `env_clear()` 대신 위험한 변수(예: `OPENAI_API_KEY`)만 삭제하는 방식을 고려했으나, 어떤 로컬 환경 변수가 부작용을 유발할지 예측 불가능하므로 Whitelist로 결정했습니다.
- **프로세스 추적(Process Tree Walker)**: PGID 방식이 아닌 PID 기반 트리 추적은 Race Condition과 PID 재사용 문제가 있어, OS 레벨의 안전한 Process Grouping을 채택했습니다.

### Consequences
- 도구 실행 중 강제 종료 시 프로세스 누수 및 포트 고스팅 완전 방지.
- 로컬 환경과의 완전한 샌드박스 격리(변수 오염 방어) 달성.
- 대규모 출력 스크립트 실행 시의 메모리 안정성 보장 및 데스크탑 환경 UX 향상.

---

## ADR-028: Phase 28 Final Touch & System Optimization (v2.0.0)
...
### Context
v1.9.0 배포 이후, 실제 운영 환경(Production)에서 장기적으로 발생할 수 있는 안정성 및 UX 결함들이 추가로 식별되었습니다. 대규모 출력 절단 시 LLM이 절단 사실을 알지 못해 생기는 인지 부조화, 멀티 터미널 환경에서 설정 파일(`config.toml`) 동시 저장 시 발생하는 파일 손상(Race Condition), Git 자동 체크포인트 레퍼런스 누적으로 인한 저장소 용량 비대화, 그리고 엄격한 환경변수 격리로 인해 발생한 사용자 커스텀 툴체인(`nvm`, `cargo` 등) 실행 불가 문제가 보고되었습니다.

### Decision
1. **Truncation Metadata 강제 주입**: `ToolResult`에 `is_truncated` 필드를 추가하고, 출력이 5MB를 넘어 절단될 경우 결과물 상단/하단에 명시적인 `[SYSTEM]` 메타데이터 텍스트를 주입하여 LLM이 이를 무조건 인지하도록 합니다.
2. **File Locking 및 원자적 쓰기**: 설정 파일 저장 시 `fs2` 기반 배타적 락(Exclusive Lock) 또는 임시 파일 쓰기 후 원자적 이름 변경(Write-and-Rename) 패턴을 사용하여 동시 쓰기 손상을 차단합니다.
3. **Git Checkpoint Pruning**: 새로운 자동 커밋 시, `refs/smlcli/checkpoints/` 하위의 레퍼런스 개수를 조회하여 오래된 50개 초과분은 즉각적으로 Prune(`git update-ref -d`) 처리합니다.
4. **Shell-Native PATH 동적 탐색**: 도구 실행 전 1회에 한해 `sh -c "echo $PATH"`를 실행시켜, 사용자의 `.zshrc` 나 `.bashrc` 등에 등록된 실제 환경의 `PATH`를 추출한 후 이를 샌드박스 환경의 기본값으로 덮어씌웁니다.

### Alternatives Considered
- **PTY Native 지원 적용 (tokio-pty/nix)**: ANSI 색상 보존과 PTY 기능을 위해 의존성을 추가하려 했으나, 크로스 플랫폼 지원의 복잡도와 안정성 리스크로 인해 일단 표준 파이프라인(Pipe) 통신을 유지하되 PTY-like ANSI 출력 보존만 시도하는 방향으로 연기/절충되었습니다.
- **Git 백그라운드 GC**: 체크포인트 용량을 줄이기 위해 전체 GC 수행을 고려했으나, 저장소의 정상적 이력마저 훼손될 수 있어 단순히 smlcli 전용 refs의 개수만 잘라내는 안전한 방법을 택했습니다.

### Consequences
- 대규모 로그 처리 시 LLM의 환각(Hallucination) 방지.
- 다중 인스턴스 환경에서도 안전한 `config.toml` 무결성 보장.
- Git 레퍼런스 누적으로 인한 IDE/Git 속도 저하 예방.
- 사용자 편의성(Local Toolchain)과 보안 샌드박스의 완벽한 균형 달성.

---

## ADR-029: Phase 29 The Final Polishing (v2.1.0)
...
### Context
v2.0.0 이후 시스템이 더욱 복잡한 환경에 배포되면서 몇 가지 코너 케이스가 발견되었습니다. 대규모 프로젝트(수백 개의 파일)에서 RepoMap 전송이 막대한 토큰 낭비를 유발하고, 터미널 창 크기 조절 시 TUI 뷰포트가 튀어버리며, `.git` 폴더는 있지만 커밋이 없는 저장소나 Detached HEAD 상태에서 Git Checkpoint 로직이 에러를 발생시키는 등의 안정성 이슈가 보고되었습니다. 또한 일부 백그라운드 프로세스가 `stdin`을 하이재킹하여 TUI 전체 이벤트를 먹통으로 만드는 치명적 결함도 제기되었습니다.

### Decision
1. **RepoMap Context Compression**: 전체 트리 구조를 전송하는 대신, 수정/접근된 파일과 그 심볼 위주로 트리를 요약 구성하며 토큰 제한 시 하위 트리를 생략 처리합니다.
2. **Scroll Anchoring**: 리사이즈 이벤트 발생 시 터미널 `height` 변화에 맞춰 기존에 보고 있던 로그 행(Line)의 절대 인덱스를 역산하여 `scroll_offset`을 실시간 보정합니다.
3. **Robust Git Detection**: 체크포인트 생성 전 `git rev-list -n 1 --all`을 통해 커밋 존재 여부를 확인하고, 실패하거나 브랜치 이름 조회가 불가능한 경우 `HEAD`나 `Initial` 이름을 사용합니다.
4. **Stdin 격리 (Nullifier)**: 모든 `tokio::process::Command`에 `.stdin(Stdio::null())`을 명시하여 자식 프로세스가 터미널 입력 스트림을 가로채지 못하도록 차단합니다.
5. **Actionable Errors**: `domain/error.rs`의 오류 체계에 `Suggestion` 필드를 추가하여, 사용자에게 기술적 단언뿐만 아니라 즉각적인 문제 해결 액션 가이드를 제공합니다.

### Alternatives Considered
- **Stdin/Stdout PTY Proxying**: 입력 하이재킹 방지를 위해 PTY 터미널을 구현하려 했으나, 비대화되는 구조를 피하기 위해 `Stdio::null()` 널링으로 심플하고 효과적인 격리 방식을 택했습니다.
- **Tree-sitter AST Parsing**: RepoMap 압축을 위해 AST를 완전히 분석하려 했으나 속도/용량 오버헤드가 커서 단순 텍스트/토큰 길이 기반의 생략 알고리즘을 우선 도입하기로 결정했습니다.

### Consequences
- 대형 프로젝트에서 토큰 비용 최대 5배 이상 절약 및 응답 속도 대폭 상승.
- 파이썬의 `input()`, 쉘의 `read` 명령어 실행 시에도 TUI가 크래시되지 않고 안정적 동작 보장.
- 창 크기 조절 중에도 포커스 잃지 않는 UX 달성.
- Git 예외 상태에서 발생하는 알 수 없는 에러 완전 차단.

---

## ADR-030: Phase 30 The Ultimate Hardening (v2.2.0)
...
### Context
v2.1.0으로 기능적인 완성도는 높아졌으나, 비-유닉스 환경(Windows)에서의 프로세스 그룹 관리 오류, 설정 쓰기 도중 발생하는 `.tmp` 파일 좀비화 현상 등 안정성과 이식성에 관련된 문제들이 지적되었습니다. 또한 향후 설정 구조의 변화에 대비한 마이그레이션 전략이 부재하고, LLM이 출력한 코드의 클립보드 연동과 문제 발생 시의 진단(Doctor) 기능이 부족하다는 피드백이 있었습니다.

### Decision
1. **Config Versioning & Auto-Migration**: `Settings` 구조체에 `version: u32` 필드를 도입. 역직렬화 성공 시 `version == 0`이면 `migrate()` 함수를 태워 v1 구조로 승격 및 기본값 적용 후 저장.
2. **Atomic Cleanup**: 초기 기동 시 작업 공간(workspace) 내의 `*.tmp` 확장자 설정 관련 파일을 탐색해 무조건 제거.
3. **Smlcli Doctor Mode**: TUI에 진입하지 않는 진단 전용 명령어를 추가해, API, Git, 권한, 터미널 환경 등에 대한 레포트를 제공.
4. **TUI Clipboard Integration**: `arboard` 라이브러리를 활용, TUI 안에서 `y` 단축키를 이용한 텍스트/코드 블록 클립보드 복사 지원.
5. **Windows Compat Separation**: `#[cfg(unix)]` / `#[cfg(windows)]` 매크로를 분리하여 플랫폼에 맞는 프로세스 종료 및 경로 파싱 로직 적용.

### Alternatives Considered
- **Strict Config Parsing (No Migration)**: 버전 필드 없이 알 수 없는 필드를 무시하는 방식도 있으나, 설정 구조의 큰 변화 시 앱 크래시나 기능 상실이 발생하므로 명시적 버전 관리가 낫다고 판단.

### Consequences
- Windows와 Unix 양쪽 환경 모두에서 안정적이고 동일한 런타임 종료 보장.
- 좀비 파일 축적으로 인한 디스크 오염 완벽히 차단.
- `doctor` 커맨드를 통해 개발자 및 사용자 자체 트러블슈팅 시간 단축.
- TUI의 코드 복사 기능으로 생산성 극대화.

---

## ADR-031: Phase 31 The Final Polish & Resilience (v2.3.0)
...
### Context
v2.2.0에서 설정 마이그레이션 및 시스템 진단 기능이 추가되었으나, 일부 엣지 케이스에서 시스템의 복원력이 부족함이 확인되었습니다. 설정 마이그레이션 실패 시 기존 파일이 손상되거나 유실될 수 있었고, `doctor` 커맨드에서 네트워크 통신 장애 시 타임아웃 없이 무한 대기하는 현상이 발생했습니다. 또한, 보안과 성능 측면에서 대규모 레포지토리에서의 RepoMap 생성 지연 및 환경 변수 제어 기능 부족 등의 문제점이 존재했습니다.

### Decision
1. **Atomic Backup & Rollback**: 설정 파일 마이그레이션 중 에러가 발생하면 무조건 원래 상태로 복구(Rollback)하는 로직 도입. `.bak` 파일로 원본을 선 복사하고 실패 시 `rename` 하여 덮어씌움.
2. **Visual Toast Notifications**: 클립보드 복사와 같은 UI 액션 성공 여부를 시각적으로 피드백하기 위해 2초 자동 만료의 Toast 알림 시스템을 레이아웃 최상단에 구현.
3. **Network Strict Timeout**: `doctor` 등 외부 API 상태 검증 로직에 `tokio::time::timeout(5초)`의 단호한 래퍼를 씌워 행(Hang) 및 무한 대기를 원천 봉쇄.
4. **Environment Whitelist**: `smlcli run` 등을 통해 도구를 띄울 때 호스트의 모든 환경 변수가 아닌 설정된 `allowed_env_vars`만 통과시키는 화이트리스트 보안 제어 로직 적용.
5. **Disk-Cached RepoMap**: 매 채팅마다 발생하는 AST 파싱(수십~수백 개의 파일) 비용 제거. 파일 수정 시간과 파일 수를 이용한 `cheap_hash`를 도입해 변동이 없을 경우 디스크에 캐싱된 JSON 맵을 로드.

### Alternatives Considered
- **In-Memory Caching (RepoMap)**: `RepoMap`을 메모리에 유지하는 방안이 있으나, 에이전트 CLI 재시작 시 성능 이점을 누릴 수 없고 세션 복원(JSONL) 시 메모리를 추가 점유하므로 디스크 캐싱(`~/.smlcli/tmp`) 방식을 선호.

### Consequences
- 환경 설정 파일 유실 위험성을 0%로 통제.
- 대규모 코드베이스에서의 AI 응답 속도 최적화.
- 사용자는 자신의 시스템 환경 변수 노출 수준을 완전히 제어할 수 있게 되어 보안성 대폭 강화.
- UI 직관성(알림) 향상.

## 36. Phase 35: System Hardening & Metadata (v2.5.0)
2026-04-22

### Context
v2.4.0 릴리즈를 준비하며 프로덕션 환경에서의 극단적 엣지 케이스들을 발견했습니다. 비정상 종료 시 ExecShell의 자식 프로세스가 고아(Orphan) 상태로 남아 시스템 리소스를 점유하는 현상, 동시성 도구 실행 시 결과가 먼저 반환된 순서대로 기록되어 타임라인 순서가 꼬이는 문제, 유니코드를 지원하지 않는 구형 터미널에서 TUI 보더가 깨지는 현상, 그리고 사용자가 보고하는 버그 리포트와 실제 배포 바이너리 간의 버전/빌드 추적이 불분명한 문제가 있었습니다.

### Decision
1. **Orphan Process Reaper**: `sysinfo` 크레이트를 도입하여 시스템의 모든 프로세스를 순회하고, 환경 변수 `SMLCLI_PID`가 삽입된 프로세스 중 부모와 일치하지 않는 고아 프로세스들을 `smlcli doctor --clean-orphans` 및 백그라운드 구동 시 자동 색출하여 SIGKILL합니다.
2. **Ordered Aggregation**: `app/mod.rs`에서 병렬 실행 중인 도구의 결과를 곧바로 렌더링하지 않고 `pending_tool_outcomes` 큐에 캐싱합니다. 완료된 인덱스가 순차적으로 정렬되었을 때에만 타임라인에 반영하여 일관된 로그 순서를 보장합니다.
3. **Adaptive ASCII Borders**: `ratatui`의 `border::Set`을 오버라이딩하여, 터미널 환경변수 `LANG`이 UTF-8을 포함하지 않거나 사용자가 `use_ascii_borders`를 명시한 경우 자동으로 `+`, `-`, `|` 기반의 ASCII 테두리로 대체합니다.
4. **DevOps Build Metadata**: `shadow-rs`를 빌드 프로세스에 연동하여 바이너리 내에 Git 커밋 해시와 빌드 타임스탬프를 주입합니다. 이를 통해 `smlcli doctor` 리포트가 어떤 빌드 환경에서 컴파일되었는지 명확히 증명합니다.
5. **Memory-Safe Log Parsing**: `SessionLogger::restore` 시 파일 전체를 `read_to_string()`으로 올리지 않고 `BufReader::lines()` 스트림 처리로 변경하여 OOM 취약점을 완화했습니다.

### Alternatives Considered
- **Process Group (PGID)**: 고아 프로세스 정리를 위해 POSIX PGID를 고려했으나, Windows 호환성과 백그라운드 데몬화 시 제어가 복잡해지므로 `SMLCLI_PID` 환경 변수 주입이라는 Cross-platform 방식을 채택했습니다.

### Consequences
- 운영 체제의 자원 누수(좀비 프로세스)를 원천 차단하여 서버 및 로컬 데스크탑 환경에서 장기 실행 안정성이 극대화되었습니다.
- 로그의 순서 보장과 바이너리 버전 추적 가능성 확보를 통해 시스템 무결성 감사(Audit) 요건을 100% 충족하게 되었습니다.

---

## ADR-032: Phase 43 MCP(Model Context Protocol) 클라이언트 통합 (v3.3.0)
...
### Context
v3.2.0까지의 도구 생태계는 `GLOBAL_REGISTRY`에 컴파일 타임에 등록된 내장 도구(ExecShell, ReadFile, WriteFile 등)만 사용 가능했습니다. 외부 MCP 서버(예: Anthropic의 `mcp-filesystem`, 커뮤니티 도구 서버 등)의 도구를 런타임에서 동적으로 활용할 수 없어 에이전트의 확장성에 한계가 있었습니다.

MCP(Model Context Protocol)는 JSON-RPC 2.0 over stdio 방식으로 외부 도구 서버와 통신하는 표준 프로토콜로, 클라이언트 측 구현을 통해 smlcli의 도구 생태계를 런타임에서 무한히 확장할 수 있게 됩니다.

### Decision

**1. mpsc 채널 기반 비동기 JSON-RPC 2.0 클라이언트 (`infra/mcp_client.rs`, 232줄)**
- 서버 프로세스를 `tokio::process::Command`로 스폰하고, stdin/stdout을 파이프로 연결합니다.
- `mpsc::channel(32)` 기반의 요청 큐와 `oneshot::channel` 기반의 응답 매칭 구조를 도입하여, Writer Task(stdin 전송)와 Reader Task(stdout 수신)를 분리합니다.
- `Arc<Mutex<HashMap<u64, oneshot::Sender>>>` 으로 pending 요청을 관리하고, `AtomicU64` 기반으로 JSON-RPC ID를 자동 채번합니다.
- 응답 대기에 `tokio::time::timeout(10초)` 래퍼를 적용하여 무한 대기를 차단합니다.
- `McpClient` 구조체에 `Debug` 및 `Clone` trait를 구현하여 `RuntimeState`에 안전하게 저장 가능하도록 했습니다.

**2. 네임스페이스 접두사 전략 (`mcp_{server}_{tool}`)**
- 외부 MCP 도구와 내장 도구 간의 이름 충돌을 방지하기 위해 `mcp_{server_name}_{tool_name}` 형식의 접두사를 부여합니다.
- LLM에게 노출되는 도구 스키마는 이 접두사가 적용된 이름을 사용하고, 실제 MCP 서버로 전달 시 `strip_prefix()`로 원본 도구명을 복원합니다.
- 초기 설계의 `{server_name}/{tool_name}` (슬래시 구분) 방식은 OpenAI tools API의 이름 규칙(`^[a-zA-Z0-9_-]+$`)과 충돌하므로 언더스코어 구분으로 변경했습니다.

**3. 런타임 캐시 기반 도구 노출 (GLOBAL_REGISTRY 미사용)**
- MCP 도구를 `GLOBAL_REGISTRY`에 동적 등록하는 대신, `RuntimeState.mcp_tools_cache: Vec<serde_json::Value>`에 OpenAI 호환 JSON Schema를 캐싱합니다.
- `chat_runtime.rs`에서 매 LLM 요청 시 `mcp_tools_cache`를 기존 도구 스키마 배열에 합류하여 전송합니다.
- 이 방식은 `GLOBAL_REGISTRY`의 `Tool` trait 구현 없이도 동적 도구 노출이 가능하며, MCP 도구의 라이프사이클을 앱 초기화 타이밍에 국한하지 않습니다.

**4. MCP 도구 전용 보안 정책 (`PermissionResult::Ask` 강제)**
- `PermissionEngine::check()`에서 `call.name.starts_with("mcp_")` 조건으로 MCP 도구를 감지하고, ShellPolicy/FileWritePolicy와 무관하게 무조건 `Ask`를 반환합니다.
- 외부 서버의 도구는 신뢰 수준을 알 수 없으므로, 사용자의 명시적 승인(y/n) 없이는 절대 실행되지 않습니다.

**5. `/mcp` 관리 명령어 (`app/command_router.rs`)**
- `/mcp list`: 현재 `config.toml`에 등록된 MCP 서버 목록 표시.
- `/mcp add <name> <command> [args...]`: 새 서버 추가 후 비동기 `save_config()` 호출. 실제 로드는 재시작 시.
- `/mcp remove <name>`: 서버 제거 후 비동기 `save_config()` 호출.

### Alternatives Considered
- **`GLOBAL_REGISTRY` 동적 등록 (`McpToolProxy` 구조체)**
  - `Tool` trait를 구현하는 프록시 구조체를 만들어 레지스트리에 직접 등록하는 방식. 그러나 `GLOBAL_REGISTRY`가 `lazy_static!` 기반 정적 구조이므로 런타임 동적 추가가 복잡하고, guard 테스트와의 호환성 문제가 발생합니다. 런타임 캐시 방식이 더 단순하고 안전합니다.
- **슬래시(`/`) 구분 네임스페이스 (`{server}/{tool}`)**
  - 가독성은 좋으나, OpenAI/Anthropic의 도구명 규칙이 영숫자와 언더스코어만 허용하므로 API 호환성을 위해 언더스코어 구분으로 변경했습니다.
- **MCP 서버 프로세스를 `App` 소유로 관리**
  - `App` 구조체가 `Child` 핸들을 직접 소유하면 `Clone` 불가로 상태 관리가 복잡해집니다. `mpsc::Sender`만 소유하는 `McpClient` 구조체를 `Clone` 가능하게 설계하여 `RuntimeState`에 안전하게 저장했습니다.

### Consequences
- 사용자는 `config.toml`의 `[[mcp_servers]]` 섹션에 서버를 등록하기만 하면, 해당 서버의 모든 도구가 AI 모델에 자동 노출됩니다.
- MCP 도구 호출 시 JSON-RPC 2.0 프로토콜을 통해 외부 서버에 위임되므로, Node.js/Python 등 다양한 언어로 작성된 MCP 서버의 도구를 네이티브하게 활용할 수 있습니다.
- 모든 MCP 도구가 `Ask` 정책으로 강제되어 보안 정책이 일관성 있게 유지됩니다.
- 서버 프로세스의 생명주기가 `tokio::spawn`으로 관리되어 메인 이벤트 루프와 독립적으로 동작합니다.

---

## ADR-033: Phase 46 Workspace-scoped Session Management (v3.6.0)
...
### Context
v3.5.0까지의 smlcli는 세션 개념이 없어 앱을 재시작하면 이전 대화가 유실되었습니다. 또한 같은 프로젝트 폴더 내에서 여러 주제의 대화를 분리하여 관리할 방법이 없어, 장기 작업 시 대화 이력이 뒤섞이는 문제가 있었습니다. opencode와 같은 최신 CLI 도구들은 세션 이어하기/새로 시작하기 기능을 제공하여 작업 흐름의 연속성을 보장하고 있었습니다.

### Decision

**1. SessionMetadata 도메인 모델 (`domain/session.rs`)**
- `SessionMetadata { id, title, workspace_root, created_at, updated_at }` 구조체를 도입하여 세션의 메타데이터를 관리합니다.
- `SessionAction` 열거형으로 세션 관련 명령어(`New`, `Resume`, `List`)를 타입 안전하게 표현합니다.

**2. SessionIndex 영속화 (`infra/session_log.rs`)**
- `sessions_index.json` 파일을 통해 워크스페이스별 세션 목록을 CRUD 관리합니다.
- `list_for_workspace(root)` API로 현재 작업 폴더의 세션만 필터링하여 반환합니다.
- `new_workspace_session()` API로 새 세션 생성 시 고유 ID 부여 + 인덱스 자동 갱신.

**3. Auto-Titling 파이프라인**
- 첫 UserMessage의 앞 50자를 자동으로 세션 제목으로 설정합니다.
- 후속 메시지는 `updated_at` 타임스탬프만 갱신하여 제목 변경 없이 활성 상태를 추적합니다.

**4. REPL 명령어 (`/session`, `/resume`, `/new`)**
- `/session`: 현재 워크스페이스의 세션 목록을 KeyValueTable로 렌더링.
- `/resume <번호>`: JSONL 로그에서 메시지 복원 + 로거 교체 + 인덱스 touch.
- `/new`: 타임라인·세션 상태·스트림 어큐뮬레이터를 초기화 후 새 세션 할당.
- SlashMenu, CommandPalette, Help 메뉴에 3건 모두 등록.

### Alternatives Considered
- **SQLite 기반 세션 저장소**
  - 쿼리 유연성은 높으나 추가 의존성(rusqlite) 도입이 필요하고, 현재의 JSONL 기반 로그 시스템과의 통합이 복잡해지므로 기각. JSON 파일 기반이 더 단순하고 디버깅이 용이함.
- **전역 세션 인덱스 (워크스페이스 무관)**
  - 모든 워크스페이스의 세션을 하나의 인덱스에서 관리하면 구현이 단순하지만, 프로젝트 간 세션이 섞여 혼란을 주므로 워크스페이스 격리 방식을 채택.
- **자동 세션 복원 (마지막 세션 자동 이어하기)**
  - UX 편의성은 높으나 "깨끗한 시작"을 원하는 사용자에게 불편하므로, 명시적 `/resume` 명령어를 통한 수동 복원으로 결정.

### Consequences
- 사용자가 프로젝트별로 여러 대화를 독립적으로 관리할 수 있게 되어 장기 작업의 연속성이 보장됨.
- Auto-Titling으로 세션 목록에서 한눈에 대화 주제를 파악 가능.
- JSONL 기반이므로 세션 데이터의 이식성(portability)과 디버깅이 용이.
- 102개 회귀 테스트 전부 통과, clippy 경고 0건 유지.

---

## ADR-034: Phase 47 Interactive Planning Questionnaire (v3.7.0)
...
### Context
PLAN 모드에서 AI가 사용자에게 요구사항을 확인할 때, 자연어로 "어떤 프레임워크를 선호하시나요?"와 같은 질문을 던지면 사용자가 자유 형식으로 답변해야 했습니다. 이 방식은 (1) AI가 구조화된 답변을 받지 못해 후속 플래닝의 정확도가 떨어지고, (2) 사용자가 무엇을 답해야 하는지 명확히 알기 어려우며, (3) 여러 질문을 한꺼번에 던질 때 UX가 혼잡해지는 문제가 있었습니다. opencode의 구조화된 플래닝 인터랙션 패턴을 참고하여, AI의 질문을 TUI 모달 폼으로 변환하는 메커니즘이 필요했습니다.

### Decision

**1. AskClarification 도구 스키마 및 하네싱**
- `tools/questionnaire.rs`에 `AskClarificationTool`을 Tool trait으로 구현하고 `GLOBAL_REGISTRY`에 등록합니다.
- 스키마에 `questions` 배열 (각 항목: id/title/options/allow_custom)과 `summary` 문자열 파라미터를 정의합니다.
- `check_permission()`은 항상 `Allow`를 반환합니다 (읽기 전용, UI 전환 도구).

**2. Action 기반 제어 (비동기 실행 우회)**
- `tool_runtime.rs`에서 `AskClarification` 도구 호출을 감지하면 비동기 실행 대신 `Action::ShowQuestionnaire` 발행.
- TUI 이벤트 루프가 Action을 수신하면 `UiState.questionnaire = Some(QuestionnaireState::new(...))` 설정.
- 답변 완료 시 `Action::QuestionnaireCompleted` → ToolResult JSON 조립 → LLM 피드백.

**3. TUI Questionnaire 모달 (`tui/widgets/questionnaire.rs`)**
- 화면 중앙 60%x40% 크기의 반투명 오버레이 모달.
- 객관식 ▸ 커서 + Cyan 하이라이트, 주관식 텍스트 입력 + ▏ 커서.
- `allow_custom` 옵션으로 "✏ 직접 입력..." 항목 자동 추가.
- 진행률 "질문 N/M" + 키보드 힌트 하단 표시.

**4. PLAN 모드 시스템 프롬프트 하네싱**
- PLAN 모드에서 시스템 프롬프트에 "사용자에게 직접 질문하지 말고 AskClarification 도구를 사용하라"는 강제 지침을 주입.
- AI가 구조화된 질문을 제출하면 자동으로 TUI 모달로 변환됨.

### Alternatives Considered
- **Composer 인라인 질문 (채팅 내 답변)**
  - AI의 질문에 사용자가 채팅으로 자유 형식 답변하는 기존 방식. 구조화되지 않아 AI가 답변을 오해하는 빈도가 높고, 여러 질문에 순차 답변하기 어려움. 기각.
- **별도 Planning UI 페이지 (전체 화면 전환)**
  - 전체 화면을 플래닝 전용 UI로 전환하는 방식. 컨텍스트 스위칭 비용이 높고, 기존 타임라인과의 연속성이 끊기므로 기각. 모달 오버레이가 더 자연스러움.
- **GLOBAL_REGISTRY에서 AskClarification을 제외하고 별도 핸들러로 처리**
  - 도구 체계의 일관성이 깨지고, LLM에게 별도 지시를 내려야 하므로 기각. Tool trait을 구현하되 실행 경로만 Action으로 우회하는 하이브리드 방식이 더 깔끔함.

### Consequences
- AI의 모호한 질문이 구조화된 폼(객관식/주관식)으로 변환되어 답변 정확도가 향상됨.
- 사용자는 키보드 ↑↓/Enter로 빠르게 답변을 선택할 수 있어 플래닝 속도가 개선됨.
- ToolResult로 조립된 답변이 LLM에 피드백되어 후속 플래닝의 품질이 높아짐.
- Action 기반 제어로 비동기 도구 실행 파이프라인을 변경하지 않고 UI 전용 도구를 안전하게 추가할 수 있는 패턴이 확립됨.
- 102개 회귀 테스트 전부 통과, clippy 경고 0건 유지.

---

## ADR-035: Phase 48 Audit Remediation (v3.7.1)
...
### Context
v3.7.1 릴리스를 앞두고 진행된 보안 및 안정성 감사에서 몇 가지 중요 결함이 발견되었습니다. 
- **설정 저장 실패 은닉**: `save_config` 호출 시 발생하는 I/O 실패가 무시되어 사용자가 알 수 없는 상태로 남는 현상.
- **MCP 타임아웃/EOF 고립**: 대기 중인 MCP 요청이 타임아웃되거나 서버 종료(EOF) 시, 펜딩 큐에서 제거되지 않아 메모리 누적이 발생하고 무한 대기하는 현상.
- **보안 우회 및 큐 고립**: `GrepSearch`가 워크스페이스 샌드박스를 우회하여 외부 경로 검색이 가능했으며, 승인 타임아웃 발생 시 큐에 남은 다음 요청이 팝업되지 않는 현상.
- **에러 재전송 루프**: 직접 셸 호출(`! command`) 실패 시 `tool_call_id`가 없는 데도 LLM으로 에러를 재전송하는 현상.

### Decision

**1. 설정 저장 실패 시 UI 에러 렌더링 강제 (`ConfigSaveFinished` 패턴)**
- `command_router.rs` 등 모든 설정 변경 경로에서 `save_config`를 비동기 호출한 뒤 결과를 `Action::ConfigSaveFinished` 이벤트로 발행하도록 일원화.
- 이벤트 루프에서 I/O 오류(`std::io::ErrorKind::StorageFull` 또는 `PermissionDenied`)를 감지하면 UI의 `err_msg` 등에 명시적으로 에러를 렌더링하도록 강제.

**2. MCP 클라이언트 펜딩 맵 구조체 승격 및 타임아웃/EOF 해소**
- `McpClient`의 `pending_requests` 맵을 구조체 필드로 승격시켜, 타임아웃 발생 시 직접 해당 요청 ID의 엔트리를 맵에서 제거.
- 서버가 예기치 않게 종료(EOF)될 경우, 맵에 남아 있는 모든 `Sender`에게 즉시 실패 응답을 통지하여 영구 대기를 방지.

**3. 샌드박스 정책 적용 및 승인 큐 승격 회귀 방지**
- `GrepSearch` 구현 시 원시 경로 대신 `file_ops::validate_sandbox()`를 호출하여 워크스페이스 외부 경로 접근을 원천 차단.
- 만료된 승인 요청 제거 후, `queued_approvals`의 0번째 요소를 확인해 다음 대기 요청이 있을 경우 즉시 `Ask` 상태로 팝업되도록 루프 패치.

**4. 직접 셸 에러 재전송 방어**
- `ToolError` 로직에서 도달한 오류 객체 중 `tool_call_id`가 누락된 경우(주로 `! command` 직접 셸 실행) LLM 재전송 큐(`pending_tool_outcomes`)에 집어넣지 않고 그대로 드롭시켜 무한 루프 방지.

### Consequences
- 모든 설정 저장 I/O 실패가 사용자에게 명확히 전달됨.
...
## ADR-037: LM Studio 런타임 연동 누락 해결 및 TUI 대시보드 정합성 교정 (v3.8.1)
...
### Context
v3.8.0 릴리스 이후 실제 LM Studio 설정을 완료하고 챗 세션을 구동할 때, LM Studio가 로컬 무인증 API임에도 불구하고 런타임 크레덴셜 분석기(`chat_runtime.rs`)에서 API Key 검색 및 파일 복호화를 시도하여 에러가 발생하거나 OpenRouter 등 엉뚱한 폴백 프로바이더로 세션이 넘어가는 런타임 오동작이 발견되었습니다.
또한, `/config` 대시보드 내 "Select Provider" 목록에 "LM Studio" 렌더링이 누락되었으며, 이로 인해 대시보드 내부의 키 이벤트 처리 인덱스와 사용자 지정 프로바이더(Custom Provider) 오프셋 불일치가 야기되어 커서 조작 시 엉뚱한 프로바이더가 활성화되는 런타임 인덱스 오류가 있었습니다.

### Decision

**1. LM Studio 런타임 크레덴셜 및 API Key 검색 무인증 예외 처리**
- `chat_runtime.rs` 내 `resolve_credentials` 함수에서 `LmStudio`일 경우, 암호화 키 저장소 조회를 우회하고 기본 모델명 및 빈 API Key(`""`) 쌍을 즉시 안전하게 반환하도록 조치했습니다.
- `resolve_credentials_for_provider` 함수에서도 `needs_key` 여부를 `false`로 기동하여 로컬 무인증 연동 가드를 성공적으로 구축했습니다.

**2. 대시보드 내 LM Studio 옵션 노출 및 TUI 팝업 인덱스 정합성 완비**
- `config_dashboard.rs` 팝업 리스트에 `"LM Studio"` 문자열 옵션을 정식 추가했습니다.
- `wizard_controller.rs` 내에서 대시보드 팝업 선택 시 `idx => 5` 분기 처리로 LmStudio를 매핑하고, 사용자 정의 프로바이더 인덱스 오프셋을 `saturating_sub(6)`으로 한 칸 보정하여 인덱스 정합성을 완비했습니다.
- `mod.rs`에서 대시보드 네비게이션 최대 높이 한계를 `5 + ...`로 상향하여 리스트 전체 렌더링이 잘림 없이 확보되도록 수정했습니다.

**3. 로컬 테스트 실행 환경의 /tmp/.git 전역 노이즈 방어 설계**
- 로컬 환경 중 `/tmp/.git` 폴더가 전역적으로 생성되어 있는 불규칙한 환경에서도 `find_workspace_root` 테스트가 오탐하여 실패하지 않도록, `workspace_utils.rs`의 `test_find_workspace_root_fallback` 검증문을 리팩토링하여 상위 부모 노드에 실제로 존재할 시 해당 지점을 정당한 반환값으로 매칭 체크하도록 테스트 견고성을 증진했습니다.

### Alternatives & Rejection Rationales (대안과 기각 사유)

* **대안 1: LM Studio 프로바이더에 대해서도 가상 API Key(예: `"lm-studio-local-key"`) 주입 강제**
  - **기각 사유**: 사용자가 로컬 오프라인 LLM을 자격 증명 없이 직관적으로 연동하려 할 때, UI 상에서 무조건 더미 키 입력을 요구하는 것은 불필요한 번거로움을 유발합니다. 또한 암호 키 스토어에 더미 값을 복호화하여 저장하고 검증하는 로직이 영속성 레이어에 추가적인 사이드 이펙트를 낳아 복잡도를 가중시키기 때문에 UX 및 코드의 단순함을 지향하고자 기각했습니다.
* **대안 2: TUI 대시보드 팝업 영역을 고정하지 않고 동적으로 자동 계산하도록 패널 시스템 개편**
  - **기각 사유**: TUI(Ratatui)의 정적 그리드 레이아웃 모델 상에서 동적 리스트 길이에 따라 창 크기를 실시간 연산하면, 렌더링 스레드의 부담을 높이고 특정 단축키 이벤트와 교차할 때 터미널 깜박임(Flicker)이 발생할 수 있습니다. 레이아웃의 고정 견고성을 무너뜨리지 않는 선에서, 정적 높이 상수를 `6`으로 1칸 확장 조율하고 매칭 오프셋 `saturating_sub(6)`을 정합하는 해결책이 아키텍처 관점에서 훨씬 실질적이고 안정적이므로 기각했습니다.

### Consequences
- 설정 위저드부터 런타임 챗 세션 구동까지 LM Studio 로컬 무인증 AI 연동이 완벽하게 정상화되었습니다.
- TUI 대시보드 내에서 커서 오동작 및 인덱스 불일치가 완전히 해소되었습니다.
- 전역 `/tmp/.git` 환경 노이즈를 완벽하게 배제하여 105개 전체 통합 및 회귀 테스트의 무결한 패스 검증을 영속적으로 보장합니다.

---

## ADR-038: ExecShell.safe_to_auto_run 신뢰 권한 경계 제한 (v3.9.0)
...
### Context
SafeOnly 모드에서 LLM은 도구 호출(`ExecShell`)을 수행할 때 `safe_to_auto_run: true` 플래그를 자체적으로 주장하여 사용자 승인(HITL)을 우회하려는 악의적 시도를 하거나 잘못 판단할 우려가 있습니다. 만약 런타임이 LLM이 생성한 `safe_to_auto_run: true`를 그대로 신뢰하여 사용자 승인 없이 임의의 셸 명령어를 실행하게 된다면, 샌드박스를 우회하거나 호스트 시스템에 파괴적인 변경을 가할 수 있는 심각한 보안 허점이 발생합니다.

### Decision
`ExecShell` 도구 호출 시, 모델이 전달하는 `safe_to_auto_run` 플래그의 권한(Authority)을 런타임에서 철저히 검증하고 제한하기로 결정했습니다.

1. **런타임 신뢰 권한 검증**:
   LLM이 `safe_to_auto_run: true`를 전달하더라도, 런타임은 해당 명령어가 내부 빌트인 안전 목록(`is_builtin_safe`)에 포함되어 있거나 사용자가 직접 설정 파일에 명시한 안전 목록(`is_custom_safe`)에 속해 있는 경우에만 이를 인정하고 자동 실행합니다.
2. **안전 목록 외 명령의 HITL 강제**:
   위의 안전 목록에 포함되지 않은 명령어는 설령 LLM이 `safe_to_auto_run: true`를 주장하더라도 런타임에서 자동으로 `false`로 덮어쓰고, 반드시 사용자에게 승인을 묻는 대화형(HITL, Human-In-The-Loop) 경로로 라우팅되도록 보장합니다.
3. **영속성 및 사양 동기화**:
   이 보안 권한 정책을 `spec.md` 및 코드(`src/tools/shell.rs`)에 완전히 반영하여, 보안 하드 바운더리를 무결하게 준수하고 이에 대한 회귀 테스트를 마련합니다.

### Alternatives Considered & Rejected
- **`safe_to_auto_run` 인자를 스키마에서 완전 제거**:
  - **기각 사유**: LLM이 빌트인 안전 명령(예: `git status`, `ls`) 등을 신뢰할 수 있게 실행하도록 제어권을 일부 제공하는 것은 자율적인 에이전트 구동 과정에서 효율성을 크게 증진시킵니다. 인자를 완전히 지우기보다는 런타임에서 유효성을 이중으로 검증하는 정책이 유연성과 보안을 동시에 만족하는 실질적인 해결책이라고 판단하여 기각했습니다.
- **사용자 전역 자동 승인 모드 도입**:
  - **기각 사유**: 샌드박스 격리가 있더라도, 무제한 셸 자동 승인은 시스템 파괴 가능성이 매우 높아 에이전트 CLI의 최소 권한 규칙(Least Privilege)에 어긋나므로 전면 배제했습니다.
