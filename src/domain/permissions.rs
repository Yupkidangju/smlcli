...
impl PermissionEngine {
    pub fn check(call: &ToolCall, settings: &PersistedSettings) -> PermissionResult {
        match call {
            ToolCall::ReadFile { .. } | ToolCall::ListDir { .. } | ToolCall::GrepSearch { .. } | ToolCall::Stat { .. } | ToolCall::SysInfo => {
            ...
            ToolCall::ExecShell { command, safe_to_auto_run, .. } => {
                match settings.shell_policy {
                    ShellPolicy::Deny => PermissionResult::Deny("Shell execution is disabled by policy.".to_string()),
                    ShellPolicy::Ask => PermissionResult::Ask,
                    ShellPolicy::SafeOnly => {
                        if *safe_to_auto_run || Self::is_safe_command(command, settings) {
                            PermissionResult::Allow
                        } else {
                            PermissionResult::Deny(format!("Command '{}' is blocked in SafeOnly mode.", command))
                        }
                    }
                }
            }
    ...
    fn is_safe_command(cmd: &str, settings: &PersistedSettings) -> bool {
        ...
        let safe_list = if os == "windows" {
            ...
            vec!["ls", "pwd", "date", "echo", "cat", "grep", "df", "free", "uname"]
        };
