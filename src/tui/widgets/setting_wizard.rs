...
pub fn draw_wizard(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    let content = match state.wizard.step {
        WizardStep::Home | WizardStep::PermissionPreset => {
            String::new()
        }
        ...
        WizardStep::ApiKeyInput => {
            format!("[Step 2] Enter API Key\n\
            Current buffer: {}\n\n\
            Press Enter to submit and fetch available models.", state.wizard.api_key_input)
        }
        WizardStep::ModelSelection => {
            if state.wizard.is_loading_models {
                ...
                format!("[Error Loading Models]\n{}\nPress Esc to restart or exit.", e)
            } else if state.wizard.available_models.is_empty() {
                "[Error Loading Models]\nNo models found. Please check API Key and Restart.".to_string()
            } else {
                ...
                for (i, m) in state.wizard.available_models[start_idx..end_idx].iter().enumerate() {
                ...
                list.push_str(format!("\n({}/{} - Use Up/Down and Enter to save completely)", state.wizard.cursor_index + 1, state.wizard.available_models.len()).as_str());
                list
            }
        }
        WizardStep::Saving => {
            "Configuration saved successfully! Press Enter one more time to start smlcli.".to_string()
        }
    };

    // TODO: 현재 입력/선택 상태를 시각적으로 더 상세히 표현할 것 (목록 포커스 효과 등)
    let paragraph = Paragraph::new(content).block(block).style(Style::default().fg(Color::Cyan));
