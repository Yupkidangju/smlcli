...
fn draw_top_bar(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    let provider = state.settings.as_ref().map(|s| s.default_provider.clone()).unwrap_or("None".to_string());
    let model = state.settings.as_ref().map(|s| s.default_model.clone()).unwrap_or("None".to_string());
    
    let text = format!(" smlcli · {}/{} · /workspace · {} · Shell Ask · {}% ctx · ✓ ", provider, model, mode_str, budget);
...
fn draw_inspector(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if let Some(tool) = &state.approval.pending_tool {
        ...
        lines.push(Line::from(vec![Span::styled("⚠️ APPROVAL REQUIRED ⚠️", Style::default().fg(Color::Yellow))]));
        ...
        if let Some(diff) = &state.approval.diff_preview {
            lines.push(Line::from(vec![Span::styled("[Diff Preview]", Style::default().fg(Color::Cyan))]));
            for l in diff.lines() {
                if l.starts_with('+') {
                    lines.push(Line::from(vec![Span::styled(l, Style::default().fg(Color::Green))]));
                } else if l.starts_with('-') {
                    lines.push(Line::from(vec![Span::styled(l, Style::default().fg(Color::Red))]));
                } else if l.starts_with('@') {
                    lines.push(Line::from(vec![Span::styled(l, Style::default().fg(Color::Cyan))]));
                } else {
        ...
        lines.push(Line::from(vec![Span::styled(">> Press 'y' to Approve, 'n' to Reject.", Style::default().fg(Color::Yellow))]));
...
fn draw_composer(f: &mut Frame, state: &AppState, area: Rect) {
    ...
    if state.fuzzy.is_open {
        ...
        let mut lines = Vec::new();
        lines.push(ratatui::text::Line::from(vec![ratatui::text::Span::raw(format!("> {}", state.fuzzy.input))]));
        
        if state.fuzzy.matches.is_empty() {
            lines.push(ratatui::text::Line::from(vec![ratatui::text::Span::styled("No files found", Style::default().fg(Color::Red))]));
        } else {
            for (i, m) in state.fuzzy.matches.iter().enumerate().take(3) {
                if i == state.fuzzy.cursor {
                    lines.push(ratatui::text::Line::from(vec![ratatui::text::Span::styled(format!("▶ {}", m), Style::default().fg(Color::Cyan))]));
                } else {
                    lines.push(ratatui::text::Line::from(vec![ratatui::text::Span::raw(format!("  {}", m))]));
                }
            }
        }
