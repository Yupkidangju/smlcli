...
pub async fn execute_tool(call: ToolCall, token: &PermissionToken) -> Result<ToolResult> {
    if let Some(tool) = crate::tools::registry::GLOBAL_REGISTRY.get_tool(&call.name) {
        ...
        if is_dest {
            safe_to_rollback = crate::tools::git_checkpoint::create_checkpoint(&cwd, tool.name()).unwrap_or(false);
        }

        let ctx = crate::tools::registry::ToolContext { token };
        let mut result = tool.execute(call.args, &ctx).await.map_err(|e| anyhow::anyhow!("{:?}", e));
        ...
        if is_dest {
            ...
            } else if should_rollback && !safe_to_rollback
