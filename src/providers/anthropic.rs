...
impl ProviderAdapter for AnthropicAdapter {
    fn validate_credentials<'a>(
        ...
        Box::pin(async move {
            ...
            if response.status().is_success() {
                ...
                Err(ProviderError::AuthenticationFailed("Invalid Anthropic API Key".into()))
            }
        })
    }
    ...
    fn chat<'a>(
        ...
        Box::pin(async move {
                    ...
                    Role::Assistant => {
                        ...
                        if let Some(tool_calls) = &msg.tool_calls {
                            for tc in tool_calls {
                                let input_json: serde_json::Value = serde_json::from_str(&tc.function.arguments).unwrap_or(serde_json::json!({}));
                                content_arr.push(serde_json::json!({
                                    "type": "tool_use",
                                    "id": tc.id,
            ...
            if !response.status().is_success() {
                ...
                return Err(ProviderError::ApiResponse { code, message: format!("Anthropic Error: {}", err_text) });
            }

            let parsed: serde_json::Value = response.json().await.map_err(|e| ProviderError::NetworkFailure(e.to_string()))?;
    ...
    fn chat_stream<'a>(
        ...
        Box::pin(async move {
            ...
            let mut full_content = String::new();
            let mut tool_calls_map: std::collections::HashMap<usize, crate::providers::types::ToolCallRequest> = std::collections::HashMap::new();
                        ...
                        match event_type {
                            "content_block_delta" => {
                                ...
                                if delta["type"] == "text_delta" {
                                    ...
                                    && let Some(partial_json) = delta["partial_json"].as_str() {
                                        let idx = parsed["index"].as_u64().unwrap_or(current_tool_idx as u64) as usize;
                            ...
                            "content_block_start" => {
                                ...
                                if cb["type"] == "tool_use" {
                                    ...
                                    tool_calls_map.insert(idx, crate::providers::types::ToolCallRequest {
