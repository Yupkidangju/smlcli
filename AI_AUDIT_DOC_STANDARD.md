...
### 3.1 Audit Report Storage and Coder Handoff

- 감사 결과는 `{project_root}/docs/audit/audit_report_[turn].md`에 저장한다.
- `[turn]`은 해당 프로젝트의 기존 감사 리포트와 충돌하지 않는 다음 감사 순번을 의미한다.
- 감사자는 감사 수행 중 소스 코드, 테스트, 설정, 구현 문서를 직접 수정하지 않는다. 수정이 필요한 내용은 finding과 감사 리포트에 기록한다.
- 각 finding에는 가능한 범위에서 증거, 기대 상태, 실제 상태, 수정 방향과 재감사 방법을 남긴다.
- 감사 완료 후, 사용자가 별도 코더 에이전트에 그대로 복사·붙여넣을 수 있도록 최신 감사 리포트 경로를 포함한 짧은 수정 프롬프트를 최종 응답의 코드 블록으로 반드시 출력한다. 감사 리포트 내부에만 기록하는 것으로 대체하지 않는다.
- 코더 인계 프롬프트는 생성한 최신 감사 리포트를 직접 참조하고, finding을 프로젝트 문서와 실제 코드에 대조하여 검토한 뒤 수정하도록 안내한다.
- 감사 리포트의 `Coder Handoff`에는 가능하면 `[turn]` 자리표시자 대신 이번 감사에서 실제 생성한 리포트 경로를 사용한다.

Example Coder Handoff Prompt:

```text
`{project_root}/docs/audit/audit_report_[turn].md`의 최신 감사 결과를 확인하고,
각 finding을 관련 프로젝트 문서와 실제 코드에 대조하여 검토한 후 필요한 수정을 수행하세요.
계약 변경이 필요한 경우 관련 문서를 먼저 갱신하고, 수정 후 관련 테스트와 검증을 실행하여 결과를 기록하세요.
```

감사 리포트는 수정 명령 자체가 아니라 검증과 인계를 위한 증거 문서다. 코더는 감사 finding을 새로운 절대 권위로 취급하지 않고 현재 사용자 요구, 프로젝트 통제 문서, 실제 코드와 증거에 대조하여 수정한다.
...
- 구현된 기능·상태·데이터·이벤트는 생성·호출 존재만으로 PASS하지 않는다. 실제 소비 경로와 후속 상태·출력·행동 변화까지 역추적하여 시스템 안에서 유효하게 연결되는지 확인한다.
- 생성되지만 소비되지 않거나, 참조되지만 실질적 영향을 만들지 못하는 고아(orphan) 요소와 단절된 인과 경로를 식별하고, 의도적으로 독립된 요소가 아니라면 finding으로 기록한다.
