...
**동작 원리:**
- `TimelineEntry`에 `depth: u8` 필드를 추가하여 들여쓰기를 조절합니다.
- 메인 답변(Planner)은 `depth: 0`을 가집니다.
- 내부적으로 실행되는 서브 태스크(Executor) 및 에러 복구 로그는 `depth: 1`을 가지며 타임라인에 `└─ ` 접두사로 렌더링됩니다.
- 사용자는 `Enter` 키를 눌러 이 아코디언을 펼치거나 접어 로그 원문을 볼 수 있습니다.
