...
**시스템 연동 애니메이션 요소 (v1.9.0 개편):**
- **터미널 타이틀 연동 (OSC 0)**: 도구 실행 중 터미널 탭 제목을 `[smlcli] Executing: <tool_name>`으로 변경하여, 사용자가 다른 화면(IDE, 브라우저)을 보고 있을 때도 백그라운드 진행 상태를 파악할 수 있도록 한다.
- **작업 표시줄 진행률 동기화 (OSC 9;4)**: 긴 빌드나 테스트 실행 중 iTerm2, VSCode 등 지원 터미널의 아이콘에 진행 상태(Indeterminate / Normal) 배지를 연동 표출한다.
...
### 8.1.1 v3.8.1 대시보드 팝업 영역 보정 및 인덱스 정합성

#### ASCII 대시보드 프로바이더 선택 팝업 레이아웃
```
┌────────────────────────────────────────────────────────┐
│ CONFIGURATION DASHBOARD (smlcli /config)               │
├────────────────────────────────────────────────────────┤
│ [System States]                                        │
│  - Active Workspace: /home/eunho1/Projects/rust/smlcli │
│  - Security Preset: Safe Starter                       │
│                                                        │
│ [Provider Settings]                                    │
│  ► Select Provider ───────────────────────────────────┐│
│    ┌────────────────────────────────────────────────┐ ││
...
#### 각 부분별 기능 상세 명세 (Functional Specifications)
* **Select Provider 팝업 패널**:
  - 사용자 유입 시 방향키 `↑`, `↓` 입력을 수신하여 활성 커서를 이동시킵니다.
  - 선택 항목은 `MAX_POPUP_HEIGHT = 6` 상수에 따라 상하단 테두리 포함 총 8라인(본문 6라인 + 커스텀 선택) 크기로 정적으로 정렬됩니다.
  - `Enter` 키 입력 시 해당 인덱스를 `wizard_controller.rs` 내 위저드 입력 이벤트 큐에 주입합니다.
* **정합성 보정 오프셋 매커니즘**:
  - `idx => 5` 매칭 분기를 통해 `ProviderKind::LmStudio`를 명확하게 UI 선택 상태와 동기화합니다.
  - `Custom...` 등 후속 사용자 정의 프로바이더 매핑 시 `idx.saturating_sub(6)` 연산을 거쳐 저장소 매핑 테이블과 1:1 바인딩이 이루어집니다.

#### 구현 시 주의사항 및 요청사항 (Design Pitfalls & Constraints)
* **팝업 잘림 현상 방지**: TUI 라이브러리(Ratatui)의 `Layout` 렌더링 영역 계산 시 `mod.rs`에서 할당하는 상한 높이(`5 + custom`)를 반드시 하드코딩하지 않고, `MAX_HEIGHT = 6`을 기점으로 삼아 패널 잘림 및 위젯 오버플로우 패닉을 방지해야 합니다.
* **커서 동기화 유지**: 사용자가 대시보드 바깥 영역을 이탈하거나 취소(`Esc`) 키를 입력하여 팝업을 닫을 시, 활성 프로바이더 인덱스 값은 기존 활성화 상태를 온전히 유지하고 갱신 이벤트를 무효화해야 합니다.
...
- `/workspace deny` **(구현 완료: 현재 root 대상)**
  - 현재 root를 Restricted + denied_roots에 등록
- `/workspace clear` **(구현 완료)**
  - 현재 root의 trust/deny 기록 초기화
- `/workspace add <path>` **(미구현 — v3.0 계획)**
  - 추가 workspace 디렉터리 등록
- `/workspace remove <path>` **(미구현 — v3.0 계획)**
  - 추가 workspace 디렉터리 제거
- `/workspace deny add <path>` **(미구현 — v3.0 계획)**
  - 접근 금지 루트 등록
- `/workspace deny remove <path>` **(미구현 — v3.0 계획)**
  - 접근 금지 루트 제거
...
- `Session`: clear, compact, sessions, quit
...
## 26. Phase 35: Adaptive UI & Locale Fallback (v2.5.0)

### 26.1 ASCII 기반 테두리 구조도

유니코드 호환성이 떨어지는 구형 터미널(TTY)이나 사용자가 명시적으로 `use_ascii_borders: true`를 지정한 경우, 모든 TUI 요소(팝업, 툴바, 탭)의 테두리가 다음의 ASCII 구조로 대체된다.
...
### 26.2 동작 규칙 및 데이터 연결
- `src/tui/widgets/mod.rs`에 정의된 `get_border_set(use_ascii_borders: bool)` 헬퍼가 호출된다.
- 이 헬퍼는 OS의 `LANG` 환경변수 또는 사용자 설정(`config.toml`의 `use_ascii_borders`)을 판별한다.
- Fallback 모드로 판별될 시 `ratatui::symbols::border::Set<'static>` 구조체의 각 모서리와 선분을 `+`, `-`, `|` 로 덮어씌워 반환한다.
- 이 테두리는 Config Dashboard, Setting Wizard, Help Overlay, Timeline 등 모든 Block 렌더링에 일관성 있게 적용된다.

---

## 27. Phase 43: MCP 클라이언트 관리 UX (v3.3.0)

### 27.1 ASCII 기반 프로젝트 디자인 구조도

```text
[ /mcp list 렌더링 ]
ℹ  Configured MCP Servers:
   - filesystem: npx -y @anthropic/mcp-filesystem /workspace
   - web-search: node mcp-search-server.js

[ /mcp add 결과 ]
ℹ  Added MCP server 'filesystem'. Restart smlcli to load it.

[ /mcp remove 결과 ]
ℹ  Removed MCP server 'filesystem'. Restart smlcli to unload it.

[ 서버 없는 상태 ]
ℹ  No MCP servers configured.

[ 타임라인: MCP 도구 호출 승인 카드 ]
┌────────────────────────────────────────────────┐
│ ⚙️ mcp_filesystem_read_file                    │
│ ────────────────────────────────────────────── │
...
#### `/mcp` 명령어 출력
- **위치**: 타임라인 영역, `SystemNotice` 블록으로 삽입.
- **렌더링 규칙**:
  - `/mcp list`: 각 서버를 `- {name}: {command} {args}` 형식으로 목록화. `palette.info` 색상 적용.
  - `/mcp add`: 추가 확인 메시지 + 재시작 안내. `palette.success` 색상.
  - `/mcp remove`: 제거 확인 메시지 + 재시작 안내. `palette.warning` 색상.
  - 서버 없음: "No MCP servers configured." `palette.muted` 색상.

#### MCP 도구 승인 카드
- **위치**: 타임라인 영역, 기존 `Approval` 카드와 동일한 UX.
- **렌더링 규칙**:
  - 도구명은 `mcp_{server}_{tool}` 전체를 표시하여 MCP 도구임을 명확히 인식.
  - 도구 인자는 JSON 형태로 축약 없이 전체 표시 (보안 투명성).
  - `[Approve: y]` / `[Deny: n]` 버튼은 기존 승인 카드와 동일.

### 27.3 동작 규칙 및 데이터 연결

- `app/command_router.rs`의 `/mcp` 분기에서 `SystemNotice` 타임라인 블록을 생성하여 출력.
- `/mcp add/remove` 시 `settings.mcp_servers` 벡터를 수정 후 `tokio::spawn`으로 비동기 `save_config()` 호출.
- MCP 서버의 실제 로드/언로드는 재시작 시에만 수행. Task M-4 E2E 테스트 완료 (v3.7.0): mock 서버 기반 initialize→list→call 왕복 검증, PermissionEngine Ask 강제, 네임스페이스 strip 왕복 확인.
- MCP 도구 승인 카드는 `PermissionEngine::check()`에서 `mcp_` 접두사 감지 시 `Ask`를 강제 반환하여 생성됨.
- 승인된 MCP 도구는 `tool_runtime.rs`에서 `McpClient::call_tool()` → JSON-RPC `tools/call` 요청으로 외부 서버에 위임.

### 27.4 구현 시 주의사항 및 요청사항
1. `/mcp add` 시 동일 이름의 기존 서버가 있으면 `retain()`으로 제거 후 새로 추가 (upsert 동작).
2. `/help` 메뉴에 `/mcp` 항목을 등록하여 사용자 발견성을 확보.
3. MCP 도구 승인 카드의 도구 인자는 민감 정보가 포함될 수 있으므로, `filter_tool_json()` 마스킹 파이프라인은 적용하지 않음 (사용자가 승인 전 전체 내용을 확인해야 하므로).
4. 서버 프로세스 크래시 시 `call_tool()`이 10초 timeout 후 `ToolError::ExecutionFailure`를 반환하므로, 별도의 UI 크래시 표시는 불필요 (기존 도구 에러 렌더링으로 충분).

---

## 28. Phase 46: Workspace-scoped Session Management UX (v3.6.0)

### 28.1 ASCII 기반 프로젝트 디자인 구조도

```text
[ /session 명령어 - 세션 목록 렌더링 ]
ℹ  Sessions in /home/user/project:
    #   제목                                    생성일               갱신일
    1   프론트엔드 컴포넌트 리팩토링 논의        2026-04-24 14:30    2026-04-24 16:45
    2   API 엔드포인트 설계                     2026-04-24 10:00    2026-04-24 12:30
    3   (현재 세션)                              2026-04-25 09:00    지금

[ /resume <번호> - 세션 전환 확인 ]
ℹ  Resumed session #1: "프론트엔드 컴포넌트 리팩토링 논의"

[ /new - 새 세션 생성 확인 ]
ℹ  New session started.
```
...
- **위치**: 타임라인 영역, `SystemNotice` 블록으로 삽입.
- **렌더링 규칙**:
  - `BlockSection::KeyValueTable`로 세션 목록을 테이블화.
  - 현재 활성 세션은 `(현재 세션)` 마커와 `palette.accent` 색상.
  - 비활성 세션은 `palette.text_secondary` 색상.
  - 세션이 없을 경우 "아직 저장된 세션이 없습니다." `palette.muted` 색상.

#### `/resume <번호>` 출력
- **위치**: 타임라인 영역, `SystemNotice` 블록.
- **렌더링 규칙**:
  - 세션 번호와 제목을 포함한 확인 메시지. `palette.success` 색상.
  - 타임라인이 초기화되고 해당 세션의 메시지가 복원됨.
  - 존재하지 않는 번호 입력 시 "해당 세션을 찾을 수 없습니다." `palette.warning` 색상.

#### `/new` 출력
- **위치**: 타임라인 영역, `SystemNotice` 블록.
- **렌더링 규칙**: "New session started." `palette.success` 색상.

### 28.3 동작 규칙 및 데이터 연결

- `app/command_router.rs`의 `/session`, `/resume`, `/new` 분기에서 라우팅.
- `infra/session_log.rs`의 `SessionIndex`가 `sessions_index.json`을 통해 영속화.
- Auto-Titling: `chat_runtime.rs`에서 첫 UserMessage의 앞 50자를 세션 제목으로 설정.
- `/resume`: 기존 세션의 JSONL 로그를 파싱하여 `session.messages`에 복원 + `logger` 교체 + `SessionIndex::touch()`.
- `/new`: 타임라인·세션 상태·스트림 어큐뮬레이터를 초기화하고 `new_workspace_session()` 호출.
- 워크스페이스 격리: `SessionIndex::list_for_workspace(root)` API가 현재 폴더의 세션만 필터링.

### 28.4 구현 시 주의사항 및 요청사항
1. Auto-Titling은 첫 메시지에만 적용. 후속 메시지는 `updated_at` 타임스탬프만 갱신.
2. `/resume` 시 메시지 복원은 동기적으로 수행하여 사용자가 즉시 이전 대화를 확인 가능.
3. SlashMenu, CommandPalette, `/help` 도움말에 세션 관련 명령어 3건 등록 필수.
4. 세션 데이터는 `~/.smlcli/sessions/` 폴더에 워크스페이스별로 격리 저장.

---

## 29. Phase 47: Interactive Planning Questionnaire UX (v3.7.0)

### 29.1 ASCII 기반 프로젝트 디자인 구조도

```text
[ Questionnaire 모달 오버레이 - 객관식 ]
╭──────────────── 📋 요구사항 확인 ──────────────────╮
│                                                     │
│  사용할 프레임워크를 선택하세요.                      │
│                                                     │
...
│      Vue.js                                         │
│      Svelte                                         │
│      ✏ 직접 입력...        ← allow_custom 옵션       │
│                                                     │
│  질문 1/3          ↑↓ 탐색 · Enter 선택 · Esc 취소  │
╰─────────────────────────────────────────────────────╯

[ Questionnaire 모달 오버레이 - 주관식/직접 입력 ]
╭──────────────── 📋 요구사항 확인 ──────────────────╮
│                                                     │
│  선호하는 프로그래밍 언어를 입력하세요.                │
│                                                     │
│  ▸ TypeScript▏                ← 텍스트 커서          │
│                                                     │
│  질문 2/3          Enter 제출 · Esc 취소            │
╰─────────────────────────────────────────────────────╯
```

### 29.2 화면별 요소 목록

#### Questionnaire 모달 오버레이 (`tui/widgets/questionnaire.rs`)
- **위치**: `layout.rs::draw()` 함수에서 help_overlay 뒤, 화면 전체를 덮는 반투명 모달.
- **크기**: 가로 60%, 세로 40%, 화면 중앙 정렬 (`centered_rect()`).
- **렌더링 규칙**:
  - **테두리**: `📋 요구사항 확인` 제목 + Rounded 또는 ASCII 테두리.
  - **질문 제목**: `palette.text_primary` Bold 스타일.
  - **객관식 옵션**: 현재 커서 위치에 `▸` 마커 + `palette.info` (Cyan) 하이라이트. 비선택 옵션은 `palette.text_secondary`.
  - **allow_custom 옵션**: 옵션 목록 끝에 `✏ 직접 입력...` 항목 추가. `palette.accent` 색상.
  - **주관식 입력 필드**: `▸ {입력텍스트}▏` 형태로 커서 표시. `palette.text_primary` 스타일.
  - **진행률**: 하단 좌측에 `질문 N/M` 표시. `palette.muted` 색상.
  - **키보드 힌트**: 하단 우측에 `↑↓ 탐색 · Enter 선택 · Esc 취소`. `palette.muted` 색상.

### 29.3 동작 규칙 및 데이터 연결

- **도구 인터셉트**: `tool_runtime.rs`에서 LLM의 `AskClarification` 도구 호출을 감지하면 비동기 실행 대신 `Action::ShowQuestionnaire` 발행.
- **UI 상태 전이**: `UiState.questionnaire: Option<QuestionnaireState>` 필드로 모달 활성화 여부 제어.
- **키 입력 핸들러**: `app/mod.rs::handle_questionnaire_key()`가 최우선 인터셉트.
  - `↑/↓`: `selected_option` 커서 이동 (wrap-around).
  - `Enter`: 객관식 선택 제출 / 주관식 텍스트 제출 / 다음 질문 전환.
  - `Esc`: 설문 취소 → `questionnaire = None` → 취소 ToolResult 전달.
  - 문자 입력: 주관식/직접 입력 모드에서 `custom_input_buffer`에 추가.
  - `Backspace`: 주관식 입력 삭제.
- **완료 시**: 모든 답변 수집 → `Action::QuestionnaireCompleted` → `AskClarificationResult` JSON 조립 → `ToolResult`로 LLM에 피드백.
- **PLAN 모드 하네싱**: `chat_runtime.rs`에서 PLAN 모드 시스템 프롬프트에 "사용자에게 직접 질문하지 말고, AskClarification 도구를 사용하라"는 강제 지침이 주입됨.

### 29.4 구현 시 주의사항 및 요청사항
1. questionnaire 모달이 활성화된 동안 다른 모든 키 입력(Composer, 슬래시 메뉴 등)은 차단됨.
2. allow_custom 옵션 선택 시 `is_custom_input_mode = true`로 전환하여 텍스트 입력 필드 표시.
