...
- `is_thinking` 플래그로 제어: `dispatch_chat_request()` 시 true, `ChatResponseOk/Err/ChatDelta(첫 토큰)` 수신 시 false.
...
현재 InspectorTab enum만 정의되어 있고 내용이 미구현된 상태.
개편 시 `widgets/inspector_tabs.rs`를 신규 생성하여 각 탭의 실제 콘텐츠를 렌더링한다.

구현 요구사항:

| 탭 | 데이터 소스 | 렌더링 |
|----|-------------|--------|
| Preview | 현재 작업 대상 파일 (최근 ReadFile 결과) | line number + 일부 범위 표시 |
| Diff | `approval.diff_preview` + 최근 적용 diff 이력 | +/- 색상 강조 |
| Search | 최근 GrepSearch 결과 | 파일별 그룹 + context lines |
| Logs | `logs_buffer` (shell/provider 원문) | 스크롤 가능한 raw 출력 |
| Recent | 최근 수정/조회 파일 목록 | 선택 시 Preview 전환 |
...
- `tui/palette.rs` 신규 파일에 상수 정의
- `layout.rs`, `widgets/*` 전체에서 하드코딩된 `Color::Yellow` 등을 palette 참조로 교체
- 상태바, 타임라인, Inspector, Composer, Wizard 모두 적용
